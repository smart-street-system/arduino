...
SoftwareSerial arduino (D1, D2); // TX and RX pins
SoftwareSerial gps_mod (D3, D4); // TX and RX pins
...
void setup() {
  ...
  gps_mod.begin(9600);
  Serial.println("GPS Integrated");
//  gps_mod.end();
//  Serial.println("GPS De-Integrated");
//  
//  arduino.begin(9600);
//  Serial.println("Ard Integrated");
//  arduino.end();
//  Serial.println("Ard De-Integrated");
}
...
//  Serial.println("GPS Integrated");
  ...
  while (gps_mod.available() > 0) //while data is available
  {
    if (gps.encode(gps_mod.read())) //read gps data
    {
      ...
      Serial.println("looping encoded");
    }
